...
import { getWhatsAppProductUrl, type DeliveryAddress } from "@/lib/whatsapp-product";
import { getSelectedAddress } from "@/lib/address-functionality";
...
export function WhatsAppInquiryButton({
  params,
  className,
  "aria-label": ariaLabel = "Inquire on WhatsApp",
  ...
  const handleClick = React.useCallback(
    (e: React.MouseEvent) => {
      ...
      if (address) {
        openWhatsApp(formatAddress(address));
      } else {
  ...
  const handleAddressSaved = React.useCallback(() => {
    const address = getSelectedAddress();
    if (address) {
      openWhatsApp(formatAddress(address));
    }
  ...
  return (
    <>
      ...
      <AddressModal
        ...
        subtitle="Please save your delivery address to send product inquiry via WhatsApp."
      />
    </>
  );
}
